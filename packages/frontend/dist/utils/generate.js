...
const { endpoint: llmEndpoint, model: llmModel, apiKey: llmKey } = {
...
async function parseJsonWithRetry(run) {
    let lastError;
...
export async function generateSeoMeta(post) {
    ...
    if (!languageCode) {
        const langResp = await comprehend.send(new DetectDominantLanguageCommand({ Text: textForLang }));
    ...
    const keyPhraseRes = await comprehend.send(new DetectKeyPhrasesCommand({
        Text: post.body,
        LanguageCode: languageCode
    }));
    const entityRes = await comprehend.send(new DetectEntitiesCommand({
        Text: post.body,
        LanguageCode: languageCode
    }));
    function cleanKeyPhrases(phrases) {
        return phrases
            .filter((p) => /^[\w\s,.-]{4,}$/.test(p) &&
            !p.includes('```') &&
            !p.match(/^[-#@*]+/) &&
    ...
    const keyPhrases = cleanKeyPhrases(keyPhraseRes.KeyPhrases?.map((k) => k.Text).filter((k) => Boolean(k)) || []);
    const entities = Array.from(new Set(entityRes.Entities?.map((e) => e.Text?.trim()).filter((e) => e && e.length > 2) || []));
    const imageMatch = post.body.match(/!\[.*?\]\((https?:\/\/[^\s)]+)\)|<img[^>]+src="(https?:\/\/[^"]+)"/);
    const ogImage = imageMatch?.[1] || imageMatch?.[2] || null;
    function inferIntentHint(targetQuery) {
        if (!targetQuery)
            return '';
...
- any text outside the JSON object.`;
    ...
    try {
        ...
        function normalizeKeywords(raw, targetQuery, options) {
            if (!raw && !targetQuery)
                return '';
        ...
        const searchIntent = validIntents.includes(json.searchIntent)
            ? json.searchIntent
            : 'informational';
        const response = {
            ...
            slug: json.slug,
            keywords: normalizeKeywords(json.keywords, post.targetQuery),
